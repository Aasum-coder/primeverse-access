    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        /* Hard mobile cap: aspect-ratio:3/4 isn't always honored on mobile browsers (tall intrinsic portrait photos can override the CSS ratio), which lets .dist-portrait grow to ~1000px tall. max-height clamps it regardless; object-fit:cover keeps the crop looking right. */
        .dist-portrait{max-width:200px;max-height:267px}}
              ...
              <div className="dist-card">
                {(() => { const pi = parseProfileImage(dist.profile_image); return pi.url ? (
                  <img className="dist-portrait" src={pi.url} alt={dist.name}
                    style={{ objectPosition: `${pi.x}% ${pi.y}%`, transform: pi.zoom !== 1 ? `scale(${pi.zoom})` : undefined, filter: pi.brightness !== 100 ? `brightness(${pi.brightness / 100})` : undefined }} />
                ) : (
      ...
      {(dist.social_telegram || dist.social_whatsapp || dist.social_tiktok || dist.social_instagram || dist.social_facebook || dist.social_snapchat || dist.social_linkedin || dist.social_youtube || dist.social_other) && (
        <div style={{ background: 'var(--deep)', padding: '1.5rem 0 0.5rem', display: 'flex', justifyContent: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          {dist.social_telegram && (
            <a href={dist.social_telegram && !/^https?:\/\//i.test(dist.social_telegram) ? 'https://' + dist.social_telegram : dist.social_telegram} target="_blank" rel="noopener noreferrer" aria-label={(() => { const m = String(dist.social_telegram || '').match(/^https?:\/\/t\.me\/([a-zA-Z0-9_]+)$/i); return m ? `Telegram: @${m[1]}` : 'Telegram' })()} title={(() => { const m = String(dist.social_telegram || '').match(/^https?:\/\/t\.me\/([a-zA-Z0-9_]+)$/i); return m ? `@${m[1]}` : 'Telegram' })()} style={{ width: 40, height: 40, borderRadius: '50%', border: '1px solid var(--gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--gold)', transition: 'box-shadow 0.3s, filter 0.3s' }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.boxShadow = '0 0 12px rgba(212,165,55,0.5)'; (e.currentTarget as HTMLElement).style.filter = 'brightness(1.3)' }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow = 'none'; (e.currentTarget as HTMLElement).style.filter = 'none' }}>
